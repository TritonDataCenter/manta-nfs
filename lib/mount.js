...
function mount(call, reply, next) {
    ...
    var stream = call.cache.mkdir(call._dirpath, call.info);

    var cb = once(function (err) {
        if (err) {
            log.warn(err, 'mount(%s): failed to list', call._dirpath);
            reply.error(nfs.MNT3ERR_SERVERFAULT);
            next(false);
        } else {
            reply.setFileHandle(call.cache.has(call._dirpath));
            log.debug('mount(%s): done', call._dirpath);
            reply.send();
            next();
        }
    });
    ...
    stream.once('flush', cb);
    call.manta.ls(call._dirpath, {_no_dir_check: true}, function (err, res) {
        if (err) {
            cb(err);
            return;
        }
