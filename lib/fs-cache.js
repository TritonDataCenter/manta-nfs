...
var murmur = require('./nfs/murmur3');
...
FsCache.prototype.readdir = function readdir(p, cb) {
    assert.string(p, 'path');
    assert.func(cb, 'callback');

    cb = once(cb);

    var info = this.cache.get(sprintf(SCHEMA.files.key, p));
    var log = this.log;

    if (!info || !info.is_directory) {
        log.debug({stat: info}, 'readdir(%s): not dir', p);
    ...
    var res = [];
    var rstream = this.createReadStream(p);
    ...
    lstream.on('data', function (l) {
        ...
        try {
            ...
            lstream.resume();
            log.error(e, 'corrupt directory entry: %s', p);
            cb(e);
        }
    });
...
FsCache.prototype._evict = function _evict(key) {
    assert.string(key);
