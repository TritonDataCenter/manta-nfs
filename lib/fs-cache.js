...
FsCache.prototype.createWriteStream = function createWriteStream(name, opts) {
    ...
    var val = {
        ...
        path: p,
        size: opts.size,
        expire: now.getTime() + (opts.ttl || this.ttl),
        dirty: opts.dirty,
        etag: opts.etag,
        md5: opts.etag
    };
    ...
    assert.ifError(joi.validate(val, SCHEMA.files.schema));
    ...
    setImmediate(function storeToLocalCache() {
...
FsCache.prototype._evict = function _evict(key) {
    ...
    this.db.get(key, function (err, val) {
        ...
        log.debug({value: val}, '_evict(%s): value retrieved');
