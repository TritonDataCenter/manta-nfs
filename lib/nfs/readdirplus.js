...
function readdirplus(call, reply, next) {
    ...
    call.fs.readdir(call._filename, function (err1, files) {
        ...
        // XXX
        // Need to handle call.dircount and call.maxcount correctly for
        // returning data. Also need to handle reading more directory info
        // based on the cookie if we couldn't return all data in one call.
        ...
        files.forEach(function (f) {
            ...
            call.fs.stat(p, function (err2, stats) {
                if (err2) {
                    log.warn(err2, 'readdirplus(%s): stat failed', p);
                    error = error || nfs.NFS3ERR_SERVERFAULT;
                    barrier.done('file: ' + p);
                } else {
                    call.fs.lookup(p, function (err3, fhandle) {
                        if (err3) {
