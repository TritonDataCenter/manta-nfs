...
function rename_get_from_dir(call, reply, next) {
    ...
    call.fs.fhandle_to_path(call.from.dir, function (err, name) {
        if (err) {
            log.warn(err, 'rename_get_from_dir(%s): fhandle_to_path notfound',
                call.from.dir);
...
function rename_get_to_dir(call, reply, next) {
    ...
    call.fs.fhandle_to_path(call.to.dir, function (err, name) {
        if (err) {
            log.warn(err, 'rename_get_to_dir(%s): fhandle_to_path notfound',
                call.to.dir);
            reply.error(nfs.NFS3ERR_STALE);
            next(false);
            return;
...
function rename_stat_from(call, reply, next) {
    ...
    call.fs.stat(call._from_filename, function (err, stats) {
        if (err) {
        ...
        if (stats.isDirectory()) {
            log.warn('rename_stat_from(%s): is a directory',
                call._from_filename);
...
function rename(call, reply, next) {
    ...
    call.fs.rename(call._from_filename, call._to_filename, function (err) {
        if (err) {
