...
function cache_directory(opts, cb) {
    ...
    assert.optionalObject(opts.info, 'options.info');
...
function ensure_file_in_cache(call, reply, next) {
    ...
    assert.ok(call.manta, 'call.manta');
    ...
    function fail(err) {
        log.debug(err, 'ensure_file_in_cache(%s): failed', p);
        reply.error(nfs.NFS3ERR_SERVERFAULT);
        next(false);
    }

    function fetch() {
        call.manta.info(p, function (err, info) {
            if (err) {
                log.debug(err, 'ensure_file_in_cache(%s): not in manta', p);
                reply.error(nfs.NFS3ERR_STALE);
                next(false);
                return;
            }

            // If it's a directory, deal with that.
            ...
            var rstream = call.manta.createReadStream(p);
            var wstream = call.cache.createWriteStream(p, {
                etag: info.etag,
                log: call.log,
                md5: info.md5,
                size: info.size || 0,
                ttl: info.headers['cache-control']
            });

            wstream.once('flush', function () {
                log.debug('ensure_file_in_cache(%s): done', p);
                next();
            });

            rstream.once('error', fail);
            wstream.once('error', fail);
            ...
            if (err.name === 'FileNotCachedError') {
                if (stats && (stats.manta || {}).is_directory) {
                    ...
                    log.debug({
                        err: err,
                        stats: stats
                    }, 'ensure_file_in_cache: cache miss; trying manta');
                    fetch();
                }
            } else {
                log.debug({
...
// function ensure_stats_in_cache(call, reply, next) {
//     assert.ok(call.cache, 'call.cache');
//     assert.ok(call.manta, 'call.manta');
//     assert.ok(call._filename, 'call._filename');
...
//     log.debug('ensure_stats_in_cache(%s): entered', call._filename);

//     if (call.cache.has(call._filename)) {
//         call.cache.stat
//     }
//     call.manta.info(call._filename, function (err, info) {
//         if (err) {
//             log.warn(err, 'ensure_stats_in_cache(%s): failed',
//                      call._filename);
//             reply.error(nfs.NFS3ERR_STALE);
//             next(false);
//             return;
...
//     call.cache.lookup(call.what.dir, function (err, name) {
//         if (err) {
//             log.warn(err, 'lookup_dir(%s): entered', call.what.dir);
//             reply.error(nfs.NFS3ERR_STALE);
//             next(false);
//         } else {
//             call._dirname = name;
...
function get_cache_by_fhandle(call, reply, next) {
    var fhandle = call._object_override || call.object;
    try {
