...
function stat_file(call, reply, next) {
    ...
    call.cache.stat(call._filename, function (err, stats) {
        if (err) {
            log.warn(err, 'stat_file(%s): entered', call._filename);
...
function stat_dir(call, reply, next) {
    ...
    call.cache.stat(call._dirname, function (err, stats) {
        if (err) {
            log.warn(err, 'stat_dir(%s): entered', call._dirname);
            reply.error(nfs.NFS3ERR_SERVERFAULT);
            next(false);
        } else {
...
function lookup(call, reply, next) {
    reply.object = call.cache.get(call._filename).fhandle;
    reply.send();
    next();
}
